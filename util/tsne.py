...
def get_features_trained_weight(model, transform_dataset):  # 透過訓練好的pth檔案進行特徵萃取
    ...
    test_dataset = WBC_all(args.data_path, args.attr_path, args.img_size, 'test', args.attrs)
    dataloader = data.DataLoader(
        test_dataset, batch_size=1024, num_workers=args.num_workers,
    ...
    for img_a, att_a, _ in progressbar(dataloader):
...
if __name__ == '__main__':
    ...
    fix_random_seeds()
    # 使用指定路徑載入訓練好的model
    model = torch.load('output/11:02AM on April 27, 2022/checkpoint/weights.1.pth')
    model.eval()
