...
def parse(args=None):
    ...
    parser.add_argument('--data', dest='data', type=str, choices=['CelebA', 'CelebA-HQ', 'wbc'], default='wbc')
...
for i in range(args.n_attrs): #
    ...
    tmp[:, i] = 1
    # seems no need~~~
    # tmp = fixed_att_a.clone() # original only one type
    # tmp[:, i] = 1 - tmp[:, i] # add one type (so will have one and original one or 0 type)
